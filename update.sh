...
dockerRepo="xolyu/docker-taiga-back-base"
latests=( $( curl -fsSL 'https://api.github.com/repos/kaleidos-ventures/taiga-back/tags' |tac|tac| \
...
for latest in "${latests[@]}"; do
    ...
    if version_greater_or_equal "$version" "$min_version"; then
        ...
        for variant in "${variants[@]}"; do
            ...
            # Copy the scripts
            for name in entrypoint.sh checkdb.py changeadminpasswd.py docker.py celery_local.py .env docker-compose.test.yml .dockerignore; do
